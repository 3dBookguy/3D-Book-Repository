...
/*
    class DWriteBox: public ControlBase
    {
    public:
        DWriteBox() : ControlBase() {}
        DWriteBox(HWND parent, int id, bool visible=true) : ControlBase(parent, id, visible) {}
        ~DWriteBox() {}

		void initialize(){
       log(L"in Dwrite Initialize");
		cTextLength_ = 0;
...
//		underline_ = FALSE;
		...
		pTextFormat_ = NULL;
		pTextLayout_ = NULL;
		HDC screen = GetDC(0);     // logical pixels per inch
		dpiScaleX_ = GetDeviceCaps(screen, LOGPIXELSX) / 96.0f;
		dpiScaleY_ = GetDeviceCaps(screen, LOGPIXELSY) / 96.0f;
		ReleaseDC(0, screen);
//		    Win::log(L"dpiScaleX_, dpiScaleY_ resized: %dx%d.", dpiScaleX_, dpiScaleX_);
		 log(L"dpiScaleX_, dpiScaleY_  %fx%f.", dpiScaleX_, dpiScaleX_);
		...
		cTextLength_ = (UINT32) wcslen(wFormattedText);
	//	delete [] wszText_; 
		wszText_ = new (std::nothrow) wchar_t[cTextLength_ + 1];
		wcscpy_s(wszText_, cTextLength_ + 1, wFormattedText);
		...
		HRESULT hr = S_OK;
        RECT rc;
//        GetClientRect(this->getHandle(), &rc);  // hope this is the right handle
        GetClientRect(getHandle(), &rc);  // hope this is the right handle

        D2D1_SIZE_F size = {static_cast<FLOAT>(rc.right),
                            static_cast<FLOAT>(rc.bottom)};
		log(L"D2D1_SIZE_F size %ix%i.", rc.right, rc.bottom);
        ...
        if (SUCCEEDED(hr))
        {
		log(L"success CreateHwndRenderTarget");
        ...
        if (SUCCEEDED(hr))
        {
		log(L"success CreateSolidColorBrush");
        pRT_->BeginDraw();
		}
    ...
    D2D1_POINT_2F origin = D2D1::Point2F(
		...
		log(L"D2D1_SIZE_F size %fx%f.", origin.x, origin.y - 500);
    pRT_->DrawTextLayout(
        origin,
        pTextLayout_,
        pBlackBrush_
