...
namespace Win
{
    ...
    public:
        ...
        virtual int createThread();                             // for WM_CREATE
      ...
      //  virtual int mouseMove(WPARAM state, int x, int y, int menu); // for WM_MOUSEMOVE: state, x, y  July17
        virtual int mouseMove(WPARAM state, int x, int y); // for WM_MOUSEMOVE: state, x, y

        virtual int cursor( bool menu, int x ); // for WM_SETCURSOR
        virtual int cursor( LPARAM lParam); // for WM_SETCURSOR
        virtual int getChar(WPARAM message, int lowWord, int highWord);		// for WM_CHAR: state, x, y
        ...
        virtual int vScroll(WPARAM wParam, LPARAM lParam, int color);      // for WM_VSCROLL
		virtual int animateControl(WPARAM button, int x, int y);
	...
	inline int Controller::animateControl(WPARAM button,  int x, int y){return 0;}
	...
	inline int Controller::createThread() { return 0; }
    ...
    inline int Controller::mouseMove(WPARAM keyState, int x, int y) { return 0; }
	 //   inline int Controller::mouseMove(WPARAM keyState, int x, int y, int menu) { return 0; }
	//July 17
    inline int Controller::cursor( bool menu, int x) { return 0; }
	inline int Controller::cursor( LPARAM lParam) { return 0; }
	inline int Controller::getChar( WPARAM message,  int lowWord, int highWord) { return 0; }
    ...
    inline int Controller::vScroll(WPARAM wParam, LPARAM lParam, int color) { return 0; }
}
#endif
