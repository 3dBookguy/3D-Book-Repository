...
///////////////////////////////////////////////////////////////////////////////
Window::Window(HINSTANCE hInst, const wchar_t* name, HWND hParent, Controller* ctrl, BOOL bisTemp, int whichWindow)
	: handle(0), instance(hInst), controller(ctrl),
...
//	winStyleEx(WS_EX_CLIENTEDGE), x(CW_USEDEFAULT), y(CW_USEDEFAULT),
   ...
   parentHandle(hParent), menuHandle(0), bisTemp(bisTemp)
{
    // copy string
	...
	//  need the tempProcedure for our fake gl context window
	if( bisTemp ) winClass.lpfnWndProc  = Win::tempProcedure;
    else winClass.lpfnWndProc  = Win::windowProcedure;       // pointer to window procedure
    winClass.cbClsExtra    = 0;
    winClass.cbWndExtra    = 0;
    winClass.hInstance     = instance;                              // owner of this class
    winClass.hIcon         = LoadIcon(instance, IDI_APPLICATION);   // default icon
    winClass.hIconSm       = 0;
	if( whichWindow == 1 ) winClass.hCursor = LoadCursor(0, IDC_IBEAM);
	else if(whichWindow == 2  ) winClass.hCursor = LoadCursor(0, IDC_ARROW);
	else if(whichWindow == 3  ) winClass.hCursor = LoadCursor(0, IDC_SIZEWE);
//    winClass.hCursor       = LoadCursor(0, IDC_IBEAM);              // default arrow cursor
    winClass.hbrBackground = (HBRUSH)GetStockObject(BLACK_BRUSH);   // default white brush
